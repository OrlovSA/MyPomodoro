...
homepath = os.path.normpath(os.getenv('USERPROFILE') + '/pomodoro/')
os.makedirs(homepath, exist_ok=True)
files_data = os.path.normpath(homepath + f'/data')
files_task = os.path.normpath(homepath + f'/task')

"""задаем размер окна и минимальные грацици"""
Window.size = (300, 350)
Window.minimum_width = 300
Window.minimum_height = 350
# print(locale.getdefaultlocale()[0]) # ru_RU
...
class OneScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        self.sound1 = SoundLoader.load('1.wav')
        self.sound2 = SoundLoader.load('2.wav')
        self.sound4 = SoundLoader.load('4.wav')
        self.sound5 = SoundLoader.load('5.wav')
        ...
        self.time_log = 0
        self.pomodoro_pb = 0
        self.dtl = f'Start {str(self.dt.hour).zfill(2)}:' \
        ...
        if os.path.exists(files_data):
            with open(files_data, 'rb') as file:
    ...
    def save_data(self):
        data = {'time_log_task': self.time_log_task,
        ...
        with open(files_data, 'wb') as file:
    ...
    def data_new(self):
        ...
        self.time_log = 0
        self.pomodoro_pb = 0
        self.dtl = f'{self.button_text_start} {str(self.dt.hour).zfill(2)}:' \
                   f'{str(self.dt.minute).zfill(2)} / '
        ...
        if self.time_25 != 0:
            ...
            if self.pomodoro_pb < 100:
                if self.time_5 == 300:
                    ...
                    Window.restore()
                    self.sound1.play()
                    self.pb.color = .31, .78, .47, 1
                    self.lb.color = .31, .78, .47, 1
                    self.but2.text = self.finish_rest_5_text
                    self.but1.text_color = .5, .5, .5, 1
                    self.but1.text = '---'
                if self.time_5 != 0:
                    self.time_5 -= 1
                    self.pbv -= 0.333
    ...
    def timer_pause(self):
        ...
        self.pb.color = .5, .5, .5, 1
        self.lb.color = .5, .5, .5, 1
    ...
    def timer_stop(self):
        if self.check1.active == True:
            self.check1.active = False
            self.it1.text = ''
            if self.check2.active == False:
                self.it1.text = self.it2.text
                self.it2.text = ''
            else:
                self.check2.active = False
                self.it2.text = ''
                if self.check3.active == False:
    ...
    def trig_button_2(self):
        if self.but2.text == self.button_text_complit or self.but2.text == self.finish_rest_5_text:
            self.timer_stop()
    ...
    def trig_chec(self, cb, lb):
        if cb.active and lb.text:
            ...
            self.time_log_task = 0
            t_data = self.dt.date()
            if os.path.exists(files_task):
                with open(files_task, 'rb') as file:
            ...
            with open(files_task, 'wb') as file:
                pickle.dump(data, file)
        else:
            cb.active = False
            self.sound8.play()
...
class HistoryScreen(OneScreen, Screen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        if locale.getdefaultlocale()[0] == 'ru_RU':
        ...
        if os.path.exists(files_task):
            ...
            for i in data:
                self.lost_history.add_widget(
                    OneLineListItem(text_color=[1, 1, 1, 1],
                                    text=f'{i[0]}'))
                for n in i[1]:
                    self.lost_history.add_widget(
                        TwoLineListItem(text=f'{n[0]}',
                                        secondary_text_color=[1, 1, 1, 1],
                                        secondary_text=f'{n[1]}'))
        else:
            self.lost_history.add_widget(
                OneLineListItem(text_color=[1, 1, 1, 1],
                                text=self.no_history_text))

    def reset(self):
        if os.path.exists(files_task):
            os.remove(files_task)
            self.lost_history.clear_widgets(children=None)
            self.lost_history.add_widget(
                OneLineListItem(text_color=[1, 1, 1, 1],
                                text=self.no_history_text))
